...
const xCoords = [...new Set(crosses.map(p => p[0]))].sort((a, b) => a - b);
const coordsX = Object.fromEntries(xCoords.map((x, i) => [x, i]));
const yCoords = [...new Set(crosses.map(p => p[1]))].sort((a, b) => a - b);
...
for (let i = 0; i < crosses.length; i++) {
    const j = (i + 1) % crosses.length;
    const point1 = crosses[i];
    const point2 = crosses[j];
...
let largestAreaPart2 = 0;
for (let i = 0; i < crosses.length; i++) {
    const first = crosses[i];
    const x1 = coordsX[first[0]];
    const y1 = coordsY[first[1]];
    mainLoop:
    for (let j = i + 1; j < crosses.length; j++) {
        const second = crosses[j];
        ...
        if (largestAreaPart2 < product) largestAreaPart2 = product;
    }
}

console.log(largestAreaPart2);